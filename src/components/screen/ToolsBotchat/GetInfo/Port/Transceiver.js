...
import {useTransceiverHooks} from './hooks/TransceiverHooks';
import Animated, {
  Easing,
  withTiming,
  useSharedValue,
...
export default function ConfigTransceiverScreen({navigation}) {
  const {
    isLoading,
    currentUser,
    isVisibleOption,
    ...
    <>
      ...
      <SafeAreaWrap
        ...
        <BannerNestedScreen
          ...
          title={`Thông Tin Module Quang ${
            pop_name?.length ? `(${pop_name})` : ''
          }`}
          ...
          {isVisibleOption && (
            <Animated.View
              ...
              <View
                ...
                <View style={tw.style(`flex-row items-center gap-2 py-2 mt-2`)}>
                  <ButtonCP
  ...
  return (
    <>
      <View style={tw.style('flex-1 relative')}>
        {fList(result_botChat).length > 0 && pop_name?.length > 0 ? (
          <ViewShot
            ref={viewShotRef}
            options={{
              quality: 0.9,
