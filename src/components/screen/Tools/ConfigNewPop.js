...
import {
  CREATE_CONFIG_NEW_POP,
  UPDATE_CONFIG_NEW_POP,
} from '../../../services/tools';
...
export default function ConfigNewPOP({
  type_job_vi = '',
  handleTimePress,
  handleTimeConfirm,
  handleDatePress,
  ...
  const isDisabledBtn =
    !nhan_su_th || !pop_name === 0 || !job_name ? true : false;
  ...
  const handleFormSubmit = () => {
    ...
    CREATE_CONFIG_NEW_POP({payload, dispatch, openNotificationToast});
  };
    ...
    <>
      <ActionSheetCP
        ...
        <View style={tw`p-2 mt-3`}>
          ...
          />
          <TextInputCP
            labelOutside="Tên POP"
            placeholder="Nhập tên POP"
            value={pop_name}
            outlinedStyle={tw`border border-[#ccc]`}
            onChange={val => {
              handleChangeForm('pop_name', val);
            }}
          />
          <CustomSelectCP
            dataList={listExe}
